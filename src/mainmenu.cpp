...
s8 trycreatefont(const char* FontName, int sizeF) {
	...
	s8 fontId = AEGfxCreateFont(fontPath.c_str(), sizeF);
	// Check if font creation failed
	if (fontId < 0)
	{
		// Try alternative fonts
		fontId = AEGfxCreateFont(std::string("../" + fontPath).c_str(), sizeF);
...
AEGfxTexture* tryLoadTexture(const char* imagePath) {
	std::string texturePath = std::string(imagePath);
	AEGfxTexture* texture = AEGfxTextureLoad(texturePath.c_str());
	...
	if (texture == nullptr)
	{
		...
		texture = AEGfxTextureLoad(std::string("../" + texturePath).c_str());
		...
		if (texture == nullptr)
		{
			...
			{
				std::cout << "Successfully loaded texture at: ../../" << texturePath << std::endl;
			}
		}
		else
		{
	...
	_In_ int       nCmdShow)
{
	...
	while (gGameRunning)
	{
		...
		{
			// ESC → free gameplay resources, go back to menu
